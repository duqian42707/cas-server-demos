# cas-server-demos

CAS 5.3.x 服务端示例项目集合

## 目录

### CAS服务端示例

- [最基本的功能](./001.cas-server-simple)
- [使用jdbc进行认证](./002.cas-server-jdbc-auth)
- [使用rest接口进行认证](./003.cas-server-rest-auth)
- [使用ldap进行认证](./015.cas-server-ldap-auth)
- [自定义认证](./004.cas-server-custom-auth)
- [自定义认证表单](./005.cas-server-custom-credential)
- [客户端服务管理](./006.cas-server-service-manage)
- [开启oauth功能](./007.cas-server-oauth2)
- [审计日志](./008.cas-server-audit)
- [将session存入redis实现分布式部署](./009.cas-server-cluster)
- [集成Apollo配置中心](./010.cas-server-apollo)
- [通过jpa进行service的管理](./011.cas-server-service-manage-jpa)
- [对外提供Restful API](./012.cas-server-restful-api)
- [使CAS实现自动登录](./013.cas-server-auto-login)
- [多因素（MFA）认证](./014.cas-server-mfa)

### CAS客户端示例

- [springboot客户端1](./101.cas-client-1)
- [springboot客户端2](./102.cas-client-2)
- [客户端提供登录界面调用CAS的API进行登录](./103.cas-client-custom-login-page)
- [进行Oauth2对接的客户端](./104.cas-client-oauth2)

### 其它

- [CAS服务管理项目](./201.cas-management)
- [以json形式存储的初始化的客户端信息](./301.services-json)
- [以json形式存储的Oauth客户端信息](./302.oauth-services-json)
